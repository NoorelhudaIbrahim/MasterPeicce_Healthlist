...
if(isset($_POST['update'])){
   ...
   $update_product = $conn->prepare("UPDATE `brands` SET brand_name = ? WHERE brand_id = ?");
   $update_product->execute([$name, $brand_id]);
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `brands` SET image_01 = ? WHERE brand_id = ?");
         $update_image_01->execute([$image_01, $brand_id]);
...
<head>
    <meta charset="utf-8">
    <title>Healthlist/Update Brand</title>
                ...
                <div class="row g-4">
                    <div class="col-sm-12 col-xl-6">
                        <div class="bg-secondary rounded h-100 p-4" style="background-color: #fff !important; ">
                            <h5 class="mb-4">Edit Your Brand</h5>
                            <?php
                                $update_id = $_GET['update'];
                                $select_brand = $conn->prepare("SELECT * FROM `brands` WHERE brand_id = ?");
                            ...
                            <input type="hidden" name="old_image_01" value="<?= $fetch_brand['image_01']; ?>">
                                <div class="mb-3">
                                    <label for="exampleInputEmail1" class="form-label">New Brand Name</label>
                                    <input type="text" name="name" required class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"  value="<?= $fetch_brand['brand_name']; ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="exampleInputEmail1" class="form-label">New Brand Image</label>
                                ...
                                <button style="background-color: yellow !important;" class="btn btn-primary"> <a href="Brands.php" class="option-btn">Go Back</a> </button>
                            </form>
                            <?php
                                    }
                                }else{
                    ...
                    <div class="col-sm-12 col-xl-6">
                        <div class="bg-secondary rounded h-100 p-4" style="background-color: #fff !important; ">
                              <?php
                                    ...
                                    $select_brand = $conn->prepare("SELECT * FROM `brands` WHERE brand_id = ?");
                                    $select_brand->execute([$update_id]);
                                    $fetch_brand = $select_brand->fetch(PDO::FETCH_ASSOC);
                                 ?>
                            <img src="./uploaded_img/<?= $fetch_brand['image_01'];?>" width="350px" height="300px" style="margin-left:100px">
                        </div>
                    </div>
                </div>
            </div>
